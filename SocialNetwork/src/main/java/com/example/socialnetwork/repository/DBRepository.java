...
public class DBRepository implements Repository{
    ...
    @Override
    public void saveMessage(User user1, User user2, String message) {

        String sql = "insert into conversations (email_sender, email_receiver, message, datetime) " +
                "values (?, ?, ?, ?)";
        String email_1 = user1.getEmail();
        String email_2 = user2.getEmail();
        try(Connection connection = DriverManager.getConnection(db_url, db_username, db_password);
            PreparedStatement ps = connection.prepareStatement(sql)){
            ...
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy'T'mm:hh");
            String stringed_time = time.format(formatter);
            ps.setString(4, stringed_time);
            ps.executeUpdate();
        } catch (SQLException e){
            e.printStackTrace();
        }
    ...
    @Override
    public ArrayList<String> getMessagesBetween(User loggedUser, User friendUser) {
        ArrayList<String> messages = new ArrayList<>();
        String email1 = loggedUser.getEmail();
        String email2 = friendUser.getEmail();
        String sql1 = "select * from conversations" + " " +
                "where (email_sender = ? and email_receiver = ?) or (email_sender = ? and email_receiver = ?)" +
                "order by dateTime";
        try(Connection connection = DriverManager.getConnection(db_url, db_username, db_password);
            ...
            while(resultSet.next()){
                String email_sender = resultSet.getString("email_sender");
                String message = resultSet.getString("message");
                messages.add(getUser(email_sender).getFirst_name());
