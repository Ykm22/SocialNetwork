...
//        Set<User> users = fileRepository.getUsers();
    ...
    public boolean testFriendship(User user1, User user2){
        ArrayList<Friendship> user1_friendships = repository.getFriendshipsOfUser(user1);
        for(Friendship friendship : user1_friendships){
            if(friendship.getEmail2().equals(user2.getEmail()) ||
                friendship.getEmail1().equals(user2.getEmail()))
    ...
    public void sendMessage(User user1, User user2, String message){
        repository.saveMessage(user1, user2, message);
        UserEntityChangeEvent event = new UserEntityChangeEvent(ChangeEventType.ADD, user1);
        notifyObservers(event);
    }

    public ArrayList<String> getMessagesBetween(User loggedUser, User friendUser) {
        return repository.getMessagesBetween(loggedUser, friendUser);
    }
}
